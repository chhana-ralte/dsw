                ...
                <p>
                    ...
                    @if ($person->valid_allotment()->valid_allot_hostel())
                        @can('update', $person->valid_allotment()->valid_allot_hostel()->hostel)
                            <a href="/person/{{ $person->id }}/person_remark/create" class="btn btn-primary btn-sm">Add
                                remark</a>
                        @endcan
                    @endif
                </p>
                                        ...
                                        @if (auth()->user()->isWarden() || auth()->user()->isAdmin())
                                            <div class="btn-group">
                                                <a class="btn btn-primary btn-sm"
                                                    href="/person_remark_detail/{{ $prd->id }}/edit">Edit</a>
                                                <button class="btn btn-danger btn-sm delete_remark_detail"
                                                    value="{{ $prd->id }}">Delete</button>
                                            </div>
                                        @endif
                                    @endforeach
                                @endif
                                    ...
                                    <div class="dropdown" style="">
                                        ...
                                        <ul class="dropdown-menu" style="z-index: 200">
                                            <li><a class="dropdown-item"
                                                    href="/person_remark/{{ $pr->id }}/person_remark_detail/create">Add
                                                    detail</a></li>
                                            <li><button class="dropdown-item delete_remark" type="button"
                                                    value="{{ $pr->id }}">Delete remark</button></li>
                                            <li><a class="dropdown-item"
                                                    href="/person_remark/{{ $pr->id }}/edit">Edit remark</button>
                                            </li>
        ...
        $(document).ready(function() {
            ...
            $("button.delete_remark_detail").click(function() {
                if (confirm("Are you sure you want to delete the remark detail?")) {
                    $("form#delete_person_remark_detail").attr('action', "/person_remark_detail/" + $(this)
                        .val());
