<x-layout>
    <x-container>
        <x-block>
            <x-slot name="heading">
                Admission status of residents of {{ $hostel->name }} Hall of Residence for the {{ $sessn->name() }}
                session.
                <p>
                        ...
                        @foreach ($allot_hostels as $ah)
                            <tr class="table-white row_{{ $ah->id }}">
                                @if ($ah->valid_allot_seat())
                                    <td>{{ $ah->valid_allot_seat()->seat->room->roomno }}/{{ $ah->valid_allot_seat()->seat->serial }}
                                    </td>
                                @else
                                    <td><i>N.A.</i></td>
                                @endif
                                ...
                                <td>
                                    @if ($ah->allotment->admission($sessn->id))
                                        ...
                                        @can('update', $hostel)
                                            <button class="btn btn-primary btn-sm btn-confirm"
                                                name="admit_{{ $ah->id }}" value="{{ $ah->id }}">Undo</button>
                                        @endcan
                                    @else
